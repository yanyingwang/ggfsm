...
(define 负􏸻
  (map (λ (N) (􏸽  ([(a b) (􏻓和𥺌 N 8)]) (~a "-" (add1 a) (add1 b) (list-ref (reverse 三十二阳卦) N)))) (build-list 32 並)))
 
(define tick-vals (􏼏  -32 33))
(define tick-text (􏿝 (reverse 负􏸻) '("〇") 正􏸻))
...
(define (layout)
    (hash
     ...
     'shapes 八卦线 'title "卦势图" 'showlegend #t 'height 1200 'colorway (current-ploty-colorway)
