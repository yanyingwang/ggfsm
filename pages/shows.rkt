               ...
               (div
                    ...
                    ,(if (string? 标)
                         ...
                         `(div ([class "row text-center justify-content-center"])
                               ...
                               (p "低价低量(市场低靡)：" ,@(􏷑 (入 (d) (􏼃 " " d " ")) (get-days 低价低量? 文)))
                               ))
                    ))
         (plotly-script 标 文)
         `(script ([type "text/javascript"] [src ,(js/ "zixuan.js")]))
         `(script "zixuanShow()")))
    (define (history-days 文)
      (􏿝 (get-days 超量超价? 文)
          (get-days 天价地量? 文)
          (get-days 天量地价? 文)
          (get-days 高价缩量? 文)
          (get-days 低价起量? 文)))
    (define (get-history-data day len)
      (攸以卦 (􏾺 (􏹅 (入 (e) (string=? (hash-ref e 'day) day)) (􏾛 日文/三年)) len)))
    (define (history-data 文)
      (let ([len (length 文)])
        (map (入 (d) (get-history-data d len))
             (history-days 文))))
    (define (history-htmls 文)
      (map (入 (data)
               (let ([day (hash-ref (first data) 'day)])
                 (cons day (html day data))))
           (history-data 文)))
